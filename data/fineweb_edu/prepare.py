"""
FineWeb-Edu dataset preparation for nanoGPT-style pretraining.

Downloads HuggingFaceFW/fineweb-edu, tokenizes it with the GPT-2 tokenizer,
and writes uint16 token shards as raw .bin files:

    python data/fineweb_edu/prepare.py

Small smoke test:

    python data/fineweb_edu/prepare.py --shard-size 2048 --max-shards 2 --num-proc 1

By default the output files are written next to this script:
...
def parse_args():
    default_data_root = os.environ.get("NANOGPT_DATA_ROOT", ".")
